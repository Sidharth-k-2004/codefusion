...
def store_selection():
    try:
        ...
        language_selected=selected_languages
        artist_selected=selected_artists
...
def get_audio_features_for_emotion(emotion):
    if emotion == "happy":
        ...
        return {}  # Default or no emotion

# Route to get recommendations based on emotion, artists, and languages
...
#             if artist_data:
#                 artist_ids.append(artist_data[0]['id'])  # Append artist ID
#         else:
...
#     params = {
#         "limit": 10,
#         "seed_artists": ','.join(artist_ids) if artist_ids else None,
#         **audio_features
#     }

#     # Make request to the Spotify Recommendations API
...
#                 'preview_url': track['preview_url'],
#                 'external_url': track['external_urls']['spotify'],
#                 'language': track['album']['available_markets']  # Optional: markets where available
#             }
#             song_recommendations.append(song_info)
...
#     data = request.json
#     current_user = users_collection.find_one({'username': new_user})
#     selected_languages = current_user.get('selectedLanguages', [])
#     selected_artists = current_user.get('selectedArtists', [])

    
#     # Check if image is present in the data
...
#     # Process each detected face and predict emotion
#     detected_emotions = []
#     for (p, q, r, s) in faces:
#         image = gray[q:q+s, p:p+r]  # Extract the face from the frame
...
#         return get_songs()
#     return jsonify({"emotions": detected_emotions}), 200
...
def detect_emotion():
    ...
    if detected_emotions:
        return get_songs(emotion=detected_emotions[0])
    else:
